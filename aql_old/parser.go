...
//matchSequence attempts to match the given sequence and, if it matches, returns the
//string values. Otherwise, returns an error of the form 'expected ...'.
func matchSequence(it []Item, seq []Token, currIndex int) ([]string, error){
	i := currIndex
	j := 0
	var ret []string
	for {
		...
		if i == currIndex + len(seq) {
			return ret, nil //matched
		}
		if it[i].ID != seq[j] {
			return nil, formatErr(fmt.Sprintf("expected %s", tokenToString[seq[j]]), it[i].LineNumber)
		}
		ret = append(ret, it[i].Content)
		j++
		i++
	}
	return ret, nil
...
func parseDescriptionBlock(it []Item, currIndex int) (b *Block, newIndex int, err error){
	if it[currIndex].ID != DESCRIPTION {
		panic("Gone nuts - not DESCRIPTION")
	}

	seq, err := matchSequence(it, []Token{DESCRIPTION, QUOTE, STRING, QUOTE} , currIndex)
	if err != nil {
